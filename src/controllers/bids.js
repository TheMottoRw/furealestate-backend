...
const load = (id = 0) => {
    let query = "select b.*,p.*,c.*,'' as password from bid b INNER JOIN properties p ON p.id=b.property_id INNER JOIN users c ON c.id=b.client_id";
    let queryId = `select b.*,p.*,c.*,'' as password from bid b INNER JOIN properties p ON p.id=b.property_id INNER JOIN users c ON c.id=b.client_id where b.id=${id} `;
...
const loadByClient = (clientId = "") => {
    let queryType = `select b.*,p.*,c.*,'' as password from bid b INNER JOIN properties p ON p.id=b.property_id INNER JOIN users c ON c.id=b.client_id  where b.client_id=${clientId}`;
    return new Promise((resolve, reject) => {
...
const loadByProperty = (propertyId = "") => {
    let queryType = `select b.*,p.*,c.*,'' as password from bid b INNER JOIN properties p ON p.id=b.property_id INNER JOIN users c ON c.id=b.client_id where b.property_id=${propertyId}`;
    return new Promise((resolve, reject) => {
        db.query(queryType, (err, res) => {
            if (err) reject(err);
            resolve(res);
