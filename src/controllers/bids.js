...
const load = (id = 0) => {
    let query = "select b.*,b.status as bid_status,p.code,p.brief_description,p.property_usage_type,p.description,p.photos,p.rooms,p.bathroom,p.parking_slots,p.sqm,p.year_built,p.price,p.address,p.status,c.name,c.phone,c.email,c.user_type from bid b INNER JOIN properties p ON p.id=b.property_id INNER JOIN users c ON c.id=b.client_id where b.status IN ('Pending','Rejected') order by b.id desc";
...
const loadSoldOrRent = (user=0,type="Admin") => {
    let queryType = `select b.*,b.status as bid_status,p.code,p.brief_description,p.property_usage_type,p.description,p.photos,p.rooms,p.bathroom,p.parking_slots,p.sqm,p.year_built,p.price,p.address,p.status,c.name,c.phone,c.email,c.user_type from bid b INNER JOIN properties p ON p.id=b.property_id INNER JOIN users c ON c.id=b.client_id where b.status ='Accepted' order by b.id desc`;
    if(type=="Landlord"){
